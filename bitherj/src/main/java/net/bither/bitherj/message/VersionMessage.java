...
/**
 * A VersionMessage holds information exchanged during connection setup with another peer. Most
 * of the fields are not
 * particularly interesting. The subVer field, since BIP 14, acts as a User-Agent string would.
 * You can and should
 * append to or change the subVer for your own software so other implementations can identify it,
 * and you can look at
 * the subVer field received from other nodes to see what they are running. If blank,
 * it means the Satoshi client.<p>
 * <p/>
 * After creating yourself a VersionMessage, you can pass it to {@link }
...
public class VersionMessage extends Message {
    ...
    /**
     * An additional string that today the official client sets to the empty string. We treat it
    ...
    public VersionMessage(int newBestHeight, boolean relayTxesBeforeFilter) {
        super();
        clientVersion = PrimerjSettings.PROTOCOL_VERSION;
        localServices = 0;
        ...
        try {
            ...
            myAddr = new PeerAddress(InetAddress.getByAddress(localhost), PrimerjSettings.getPort(Utils.isTestNet()), 0);
            theirAddr = new PeerAddress(InetAddress.getByAddress(localhost),
                    PrimerjSettings.getPort(Utils.isTestNet()), 0);
    ...
    public void parse() throws ProtocolException {
        ...
        myAddr = new PeerAddress(bytes, cursor, 0, 26);
        cursor += myAddr.getMessageSize();
        theirAddr = new PeerAddress(bytes, cursor, 0, 26);
        cursor += theirAddr.getMessageSize();
        // uint64 localHostNonce  (random data)
        // We don't care about the localhost nonce. It's used to detect connecting back to
        // yourself in cases where
        // there are NATs and proxies in the way. However we don't listen for inbound connections
        // so it's irrelevant.
        readUint64();
        try {
    ...
    @Override
    public int hashCode() {
        return (int) bestHeight ^ clientVersion ^ (int) localServices ^ (int) time ^ subVer
                .hashCode() ^ myAddr.hashCode() ^ theirAddr.hashCode() * (relayTxesBeforeFilter ?
                1 : 2);
    }
    ...
    /**
     * Appends the given user-agent information to the subVer field. The subVer is composed of a
     * series of
     * name:version pairs separated by slashes in the form of a path. For example a typical
     * subVer field for BitCoinJ
     * users might look like "/BitCoinJ:0.4-SNAPSHOT/MultiBit:1.2/" where libraries come further
     * to the left.<p>
     * <p/>
     * There can be as many components as you feel a need for, and the version string can be
     * anything, but it is
     * recommended to use A.B.C where A = major, B = minor and C = revision for software
     * releases, and dates for
     * auto-generated source repository snapshots. A valid subVer begins and ends with a slash,
     * therefore name
     * and version are not allowed to contain such characters. <p>
     * <p/>
     * Anything put in the "comments" field will appear in brackets and may be used for platform
     * info, or anything
     * else. For example, calling <tt>appendToSubVer("MultiBit", "1.0",
     * "Windows")</tt> will result in a subVer being
     * set of "/BitCoinJ:1.0/MultiBit:1.0(Windows)/. Therefore the / ( and ) characters are
     * reserved in all these
    ...
    /**
     * Returns true if the clientVersion field is >= Pong.MIN_PROTOCOL_VERSION. If it is then
     * ping() is usable.
     */
    ...
    /**
     * Returns true if the clientVersion field is >= FilteredBlock.MIN_PROTOCOL_VERSION. If it is
     * then Bloom filtering
     * is available and the memory pool of the remote peer will be queried when the downloadData
     * property is true.
     */
